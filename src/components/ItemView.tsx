...
const FallingItemContainerDiv = styled.div<FallingItemContainerProps>`
    position: absolute;
    top: ${(props) => (props.hasReachedGoalLine ? 'unset' : `${props.offsetY}px`)};
    ...
    const getItemDivBasedOnShape = (shape: FallingItemShape) => {
        switch (shape) {
            case FallingItemShape.circle:
                return <CircleItemDiv itemColor={itemColor}>{weightIndicatorText}</CircleItemDiv>;
            case FallingItemShape.rectangle:
                return <SquareItemDiv itemColor={itemColor}>{weightIndicatorText}</SquareItemDiv>;
            case FallingItemShape.triangle:
                return <TriangleItemDiv itemColor={itemColor}>{weightIndicatorText}</TriangleItemDiv>;
        }
    };
